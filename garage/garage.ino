...
void handleRoot() {
  ...
  httpResponse += "<html lang=\"de\">\n";
  httpResponse += "<head>\n";
  httpResponse += "<META HTTP-EQUIV=\"Content-Type\" CONTENT=\"text/html; charset=iso-8859-1\"/>\n";
  httpResponse += "<meta name=\"viewport\" content=\"width=device-width, initial-scale=1\">\n";
  httpResponse += "<title>Steuerung Garagentor</title>\n";
  httpResponse += "<link rel=\"shortcut icon\" type=\"image/x-png\" href=\"";
  //Image copied from Wikimedia Commons, see https://commons.wikimedia.org/wiki/File:Ic_settings_remote_48px.svg
  ...
  httpResponse += "function openGarage() { \
      var xhttp = new XMLHttpRequest(); \
      xhttp.open(\"GET\", \""+command+"\", true); \
      xhttp.send(); \
      if(new URLSearchParams(location.search).has(\"log\")) setTimeout(function() {location.reload();}, 1000); \
    }\n";
  httpResponse += "</script>\n";
  ...
  httpResponse += "<body>\n";
  httpResponse += "<h1>Garagentor</h1>\n";
  httpResponse += "<a onClick='openGarage()'/>auf/zu</a>\n";
  ...
  server.send(200, "text/html", httpResponse);
}
